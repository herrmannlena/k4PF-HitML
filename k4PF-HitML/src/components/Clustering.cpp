...
std::vector<float> Clustering::compute_local_density(const std::vector<float>& distances,
                                                     const std::vector<float>& energies, std::size_t n_points) const {
...
Clustering::distance_to_larger_density(const std::vector<float>& distances, const std::vector<float>& rho,
  ...
  std::stable_sort(order.begin(), order.end(),
                   [&rho](std::size_t lhs, std::size_t rhs) { return rho[lhs] > rho[rhs]; });
...
std::vector<int64_t> Clustering::cluster_centers(const std::vector<float>& rho, const std::vector<float>& delta) const {
...
std::vector<int64_t> Clustering::assign_cluster_id(const std::vector<float>& rho, const std::vector<int64_t>& nearest,
  ...
  std::iota(order.begin(), order.end(), 0);
  std::sort(order.begin(), order.end(), [&rho](std::size_t lhs, std::size_t rhs) { return rho[lhs] > rho[rhs]; });
...
torch::Tensor Clustering::get_clustering(const std::vector<float>& output_vector, const std::vector<float>& energies,
  ...
  torch::Tensor output_model_tensor =
      torch::from_blob(const_cast<float*>(output_vector.data()), {N, 4}, torch::dtype(torch::kFloat32)).clone();
  ...
  if (dumpEventIdx >= 0) {
    ...
    for (std::size_t i = 0; i < n_points; ++i) {
      out << rho[i] << " " << delta[i] << " " << nearest[i] << " " << ids[i] << " " << labels[i] << "\n";
    }

    std::ofstream centersOut("dump/cpp_event_" + std::to_string(dumpEventIdx) + "_dpc_centers.txt");
...
torch::Tensor Clustering::remove_bad_tracks_from_cluster(const torch::Tensor& labels_in,
  ...
  if (hit_type.size() != static_cast<std::size_t>(n_nodes) || e_hits.size() != static_cast<std::size_t>(n_nodes) ||
